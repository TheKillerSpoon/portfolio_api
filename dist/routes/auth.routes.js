...
authRoute.post("/auth/signin", async (req, res) => {
    try {
        console.log("Received sign-in request:", await Collection.findOne({ username: req.body.username }));
        ...
        const validPass = await bcryptjs.compare(req.body.password, user.hashedPassword);
        if (!validPass) {
            return res.status(401).send({
                status: "error",
                message: "Invalid email or password",
